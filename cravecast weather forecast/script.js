...
api_key = 'a12d15c325fe15a5448326f600002d13',
...
function getWeatherDetails(name, lat, lon, country, state){
    ...
    fetch(WEATHER_API_URL).then(res => res.json()).then(data => {
        ...
        currentWeatherCard.innerHTML = `
			...
			<div class="card-footer">
				...
				<p><i class= "fa-light fa-location-dot"></i>A${name}, ${country}</p>
			</div>
        `;
    ...
    fetch(FORECAST_API_URL).then(res => res.json()).then(data =>{
        ...
        for(i = 1; i < fiveDaysForecast.length; i++){
            let date = new Date (fiveDaysForecast[i].dt.txt);
            fiveDaysForecastCard.innerHTML += `
        ...
        alert('Failed t0 fetch weather forecast');
    });
}
...
locationBtn.addEventListener('click', getCityCoordinates);